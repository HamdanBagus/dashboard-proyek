    ...
    <x-slot name="header">
        <div class="flex items-center gap-2">
            ...
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                <a href="{{ route('projects.photo.index', $project->id) }}" class="text-[#F8931F] hover:underline transition">
                    Laporan Foto Udara
                </a>
    ...
    <div class="pt-6 pb-12 max-w-9xl mx-auto sm:px-6 lg:px-8 space-y-6">
        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-xl border border-gray-200">
            ...
            <div class="p-6 border-b border-gray-100">
                <form action="{{ route('photo-progress.store', $hamparan->id) }}" method="POST" class="flex flex-col md:flex-row gap-4 items-end bg-[#F4F7F6] p-5 rounded-xl border border-[#144C4D]/10 shadow-sm">
                    @csrf
        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-xl border border-gray-200 mb-12">
            ...
            <div class="p-6 border-b border-gray-100">
                <form action="{{ route('photo-outputs.store', $hamparan->id) }}" method="POST" class="flex flex-col md:flex-row gap-4 items-end bg-[#F4F7F6] p-5 rounded-xl border border-[#144C4D]/10 shadow-sm">
                    @csrf
                    <div class="flex-1 w-full">
                        <label class="block text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-1.5">Target Jenis Output / Nama File</label>
                        <input type="text" name="filename" list="jenis_output" placeholder="Ketik atau pilih dari daftar..." class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-[#144C4D] focus:ring-[#144C4D] sm:text-sm font-medium" required>
                        ...
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            <td class="px-6 py-4 text-center">
                                <div class="flex justify-center items-center gap-2">
                                    ...
                                    <form action="{{ route('photo-outputs.destroy', $out->id) }}" method="POST" onsubmit="return confirm('Hapus target output ini?');">
                                        @csrf @method('DELETE')
                                        <button type="submit" class="text-red-500 hover:text-white hover:bg-red-500 font-bold border border-red-500 p-1.5 rounded-lg transition shadow-sm" title="Hapus">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                                        </button>
