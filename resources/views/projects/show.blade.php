                        ...
                        @if(is_array($project->planned_uavs) && count($project->planned_uavs) > 0)
                            <ul class="space-y-1.5">
                                @foreach($project->planned_uavs as $uav)
                                    <li class="flex flex-col items-start gap-2.5 border-b border-gray-100 py-4 last:border-0 last:pb-0 first:pt-0">
                                        <span class="text-sm font-bold text-gray-800 leading-snug w-full">
                                            {{ $uav['id'] ?? 'Unknown' }}
                                        </span>
                                        <span class="text-[#F8931F] bg-orange-50 px-2 py-0.5 rounded-md text-[10px] uppercase tracking-widest border border-orange-100 font-bold shrink-0">
                                            {{ $uav['qty'] ?? 0 }} Unit
                                        </span>
                                        <a href="{{ route('projects.uav-log.show', ['project' => $project->id, 'uav_name' => $uav['id']]) }}" 
                                        class="inline-flex items-center justify-center gap-1.5 text-[10px] bg-[#144C4D] text-white px-3 py-2 mt-1 rounded-lg hover:bg-[#0c2e2e] transition-all shadow-sm font-bold tracking-widest uppercase group">
